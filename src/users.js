...
UserService.prototype.renderAllUsers = function(users) {
    ...
    users.forEach(user => {
        ...
        const showSelectedUser = async () => {
         ...
         /**
                Ваш підхід до оновлення moreInfoArea.innerHTML функціонально працює і призводить до оновлення всіх даних одночасно.
                (!) Однак з точки зору оптимізації DOM і роботи з ним, цей метод не є найефективнішим.
                Коли ви встановлюєте innerHTML, весь вміст moreInfoArea перетворюється в рядок HTML і >>> повністю замінюється на новий вміст <<< , 
                ...
                (тут думаємо що якщо будемо масштабувати і складність апки виросте)
